...
describe('The OVE Core server', () => {
    ...
    it('should be able to successfully read, update and delete sections by group, with an app', async () => {
        ...
        await request(app).get('/sections?groupId=1')
            .expect(HttpStatus.OK, JSON.stringify([{ id: 1, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8082' } }]));
        ...
        await request(app).get('/sections?groupId=2')
            .expect(HttpStatus.OK, JSON.stringify([
                { id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8081' } },
                { id: 1, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8082' } }
            ]));

        const mockCallback = jest.fn(x => x);
        const OLD_LOG_WARN = log.warn;
        log.warn = mockCallback;
        await request(app).get('/sections?groupId=1&geometry=100')
            .expect(HttpStatus.OK, JSON.stringify([{ id: 1, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8082' } }]));
        ...
        await request(app).get('/sections/0').expect(HttpStatus.OK, JSON.stringify({ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8081' } }));
        await request(app).post('/sections/moveTo?groupId=1').send({ space: 'TestingNine' }).expect(HttpStatus.OK, JSON.stringify({ ids: [1] }));
        await request(app).post('/sections/moveTo?groupId=2').send({ space: 'TestingNine' }).expect(HttpStatus.OK, JSON.stringify({ ids: [0, 1] }));

        const scope1 = nock('http://localhost:8081').post('/instances/0/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scope2 = nock('http://localhost:8082').post('/instances/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
    ...
    it('should be able to successfully read, update and delete sections by space, with an app', async () => {
        ...
        await request(app).get('/sections?space=TestingNine&geometry=100')
            .expect(HttpStatus.OK, JSON.stringify([{ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8081' } }]));
        log.warn = OLD_LOG_WARN;
        expect(mockCallback.mock.calls[0][0]).toBe('Ignoring invalid geometry:');
    ...
    it('should be able to successfully read and update all sections, with an app', async () => {
        ...
        await request(app).get('/sections')
            .expect(HttpStatus.OK, JSON.stringify([{ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8081' } }]));
        ...
        await request(app).post('/sections/transform').send({ scale: { x: 10, y: 1 } }).expect(HttpStatus.OK, JSON.stringify({ ids: [0] }));
        await request(app).get('/sections/0')
            .expect(HttpStatus.OK, JSON.stringify({ id: 0, x: 10, y: 0, w: 100, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8081' } }));
        ...
        await request(app).delete('/sections').expect(HttpStatus.OK, Utils.JSON.EMPTY);
        await request(app).get('/sections').expect(HttpStatus.OK, Utils.JSON.EMPTY_ARRAY);
    });

    it('should be able to successfully pre-load application state during creation of a section', async () => {
        const scope = nock('http://localhost:8081').post('/instances/0/state', JSON.stringify({ foo: 'bar' })).reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scopeFlush = nock('http://localhost:8081').post('/instances/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
    ...
    it('should be able to successfully load named application state during creation of a section', async () => {
        const scope = nock('http://localhost:8081').post('/instances/0/state', '*').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scopeFlush = nock('http://localhost:8081').post('/instances/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        ...
        await request(app).get('/sections/0')
            .expect(HttpStatus.OK, JSON.stringify({ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8081', state: 'foo' } }));
    ...
    it('should be able to successfully cache application states during creation of a section', async () => {
        const scope1 = nock('http://localhost:8081').post('/states/dummy', JSON.stringify({ foo: 'bar' })).reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scope2 = nock('http://localhost:8081').post('/states/newDummy', JSON.stringify({ alpha: 'beta' })).reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scopeFlush = nock('http://localhost:8081').post('/instances/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
    ...
    it('should be able to update the app of a section', async () => {
        ...
        await request(app).get('/sections/0')
            .expect(HttpStatus.OK, JSON.stringify({ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8082' } }));
    ...
    it('should be able to set opacity of an app', async () => {
        ...
        await request(app).get('/sections/0')
            .expect(HttpStatus.OK, JSON.stringify({ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8081', opacity: '1.0' } }));
        ...
        // dimensions should not change as the update only changes the app.
        await request(app).get('/sections/0')
            .expect(HttpStatus.OK, JSON.stringify({ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8082', opacity: '0.1' } }));
        ...
        // dimensions should not change as the update only changes the app.
        await request(app).get('/sections/0')
            .expect(HttpStatus.OK, JSON.stringify({ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingFour', app: { url: 'http://localhost:8082', opacity: '0.2' } }));
    ...
    it('should be able to successfully pre-load application state during an update of a section', async () => {
        ...
        nock('http://localhost:8081').post('/instances/0/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        nock('http://localhost:8082').post('/instances/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scope = nock('http://localhost:8082').post('/instances/0/state', JSON.stringify({ foo: 'bar' })).reply(HttpStatus.OK, Utils.JSON.EMPTY);
    ...
    it('should be able to successfully load named application state during an update of a section', async () => {
        ...
        nock('http://localhost:8081').post('/instances/0/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        nock('http://localhost:8082').post('/instances/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scope = nock('http://localhost:8082').post('/instances/0/state', '*').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        ...
        await request(app).get('/sections/0')
            .expect(HttpStatus.OK, JSON.stringify({ id: 0, x: 10, y: 0, w: 10, h: 10, space: 'TestingNine', app: { url: 'http://localhost:8082', state: 'foo' } }));
    ...
    it('should be able to successfully cache application states during an update of a section', async () => {
        ...
        nock('http://localhost:8081').post('/instances/0/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        nock('http://localhost:8082').post('/instances/flush').reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scope1 = nock('http://localhost:8082').post('/states/dummy', JSON.stringify({ foo: 'bar' })).reply(HttpStatus.OK, Utils.JSON.EMPTY);
        const scope2 = nock('http://localhost:8082').post('/states/newDummy', JSON.stringify({ alpha: 'beta' })).reply(HttpStatus.OK, Utils.JSON.EMPTY);
    ...
    it('should not fetch app state if it is not requested', async () => {
        ...
        await request(app).post('/section').send(state).expect(HttpStatus.OK);

        const scope = nock('http://localhost:8081').get('/instances/0/state').reply(HttpStatus.OK, JSON.stringify({ foo: 'bar' }));
        let res = await request(app).get('/sections/0');

        expect(res.statusCode).toEqual(HttpStatus.OK);
        expect(JSON.parse(res.text).app.states).toBeUndefined();
        expect(scope.isDone()).not.toBeTruthy();

        res = await request(app).get('/sections');

        expect(res.statusCode).toEqual(HttpStatus.OK);
        expect(JSON.parse(res.text)[0].app.states).toBeUndefined();
        expect(scope.isDone()).not.toBeTruthy();
    ...
    it('should fetch app state if it is requested when fetching a single section', async () => {
        ...
        await request(app).post('/section').send(state1).expect(HttpStatus.OK);
        await request(app).post('/section').send(state2).expect(HttpStatus.OK);
        ...
        let res = await request(app).get('/sections/0?includeAppStates=true');

        expect(res.statusCode).toEqual(HttpStatus.OK);
        expect(JSON.parse(res.text).app.states.load).toEqual(appStatePayload);
    ...
    it('should fetch app state if it is requested when fetching all sections', async () => {
        ...
        const scope1 = nock('http://localhost:8081').get('/instances/0/state').reply(HttpStatus.OK, JSON.stringify(appStatePayload1));
        const scope2 = nock('http://localhost:8082').get('/instances/1/state').reply(HttpStatus.OK, JSON.stringify(appStatePayload2));
        const res = await request(app).get('/sections?includeAppStates=true');

        expect(res.statusCode).toEqual(HttpStatus.OK);
        expect(JSON.parse(res.text)[0].app.states.load).toEqual(appStatePayload1);
        expect(JSON.parse(res.text)[1].app.states.load).toEqual(appStatePayload2);
    ...
    it('should not return app state if there was an error when fetching it, if it is requested when fetching a single section', async () => {
        ...
        const scope = nock('http://localhost:8081').get('/instances/0/state').reply(HttpStatus.NOT_FOUND);
        const res = await request(app).get('/sections/0?includeAppStates=true');
    ...
    it('should not return app state if there was an error when fetching it, if it is requested when fetching all sections', async () => {
        ...
        const scope1 = nock('http://localhost:8081').get('/instances/0/state').reply(HttpStatus.NOT_FOUND);
        const scope2 = nock('http://localhost:8082').get('/instances/1/state').reply(HttpStatus.NOT_FOUND);
        const res = await request(app).get('/sections?includeAppStates=true');
    ...
    it('should not return app state if the section had no app, if it is requested when fetching a single section', async () => {
        ...
        expect(res.statusCode).toEqual(HttpStatus.OK);
        expect(JSON.parse(res.text).app).toBeUndefined();
        expect(scope.isDone()).not.toBeTruthy();
    ...
    it('should not return app state if the section had no app, if it is requested when fetching all sections', async () => {
        ...
        expect(res.statusCode).toEqual(HttpStatus.OK);
        expect(JSON.parse(res.text)[0].app).toBeUndefined();
        expect(scope.isDone()).not.toBeTruthy();
    ...
    it('should do nothing new if app state was requested but no apps existed when fetching all sections', async () => {
        await request(app).get('/sections?includeAppStates=true').expect(HttpStatus.OK, Utils.JSON.EMPTY_ARRAY);
    });
